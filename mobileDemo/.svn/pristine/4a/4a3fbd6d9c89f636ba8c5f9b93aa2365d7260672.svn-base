...
(function() {
	...
	Morris.Donut = (function(_super) {
		...
		Donut.prototype.setLabels = function(label1, label2) {
			...
			maxWidth = 10 * inner;
			maxHeightTop = inner / 2.5;
			//圆环中间中文字体的大小
			maxHeightBottom = inner / 2;
			//圆环中间数字的大小
			this.text1.attr({
				text : label1,
				transform : ''
	...
	Morris.DonutSegment = (function(_super) {
		...
		function DonutSegment(cx, cy, inner, outer, p0, p1, color, backgroundColor, index, raphael) {
			...
			//默认圆环的半径
			this.selectedPath = this.calcSegment(this.inner - 1.5, this.inner + this.outer);
			//鼠标附上去之后的半径
			this.hilight = this.calcArc(this.inner + this.outer + 1.5);
			//线环的半径
		}
